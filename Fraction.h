...
private:
	...
	Polynom dividePolynoms(Polynom divisible, Polynom divider, RETURNTYPEDIVIDEPOLYNOMS returnType) {
		...
		for (long long i = orderPower; i >= 0; --i) {
			...
			diffNumenator = multiplicationPolynoms(divider, { multiplier });
			tempNumenator = substractionPolynoms(tempNumenator, diffNumenator);
...
public:
	...
	void addition(const Fraction& other) {
		...
		if (denumerator != other.denumerator) {
			otherNumerator = multiplicationPolynoms(other.numerator, denumerator);
			numerator = multiplicationPolynoms(numerator, other.denumerator);
			denumerator = multiplicationPolynoms(other.denumerator, denumerator);
		}

		numerator = additionPolynoms(numerator, otherNumerator);
	...
	void substraction(const Fraction& other) {
		...
		numerator = substractionPolynoms(numerator, otherNumerator);
	...
	void multiplication(const Fraction& other) {
		numerator = multiplicationPolynoms(numerator, other.numerator);
		denumerator = multiplicationPolynoms(other.denumerator, denumerator);
	...
	void division(const Fraction& other) {
		numerator = multiplicationPolynoms(numerator, other.denumerator);
		denumerator = multiplicationPolynoms(other.numerator, denumerator);
	...
	bool isInteger(const Polynom& polynom) {
		bool isInteger = true;
		for (auto& member : polynom) {
			if (member.second != 0 && member.first != 0) {
				isInteger = false;
			}
		}
		return isInteger;
	...
	void normilizeFraction() {
		...
		numerator = dividePolynoms(numerator, p, QUOTIENT);
		denumerator = dividePolynoms(denumerator, p, QUOTIENT);
	}
